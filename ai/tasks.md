...
- 写字板的远程可用性与隐私隔离要同时设计
- 真实记录读取、正文保存、meta 修改都以“能否连接本地 agent”为前提，而不是以网页部署位置为准
...
- 页面启动时从接口拉取 metas
- 打开某一天时从接口拉取 entry
- `localStorage` 仅保留草稿缓存、待同步缓存或临时状态
- 连不上本地 agent 时，页面明确降级为缓存模式，而不是继续把浏览器本地数据当正式数据源
- 待同步内容进入可审查队列，而不是在恢复连接后自动写入本地数据库
- 明确哪些内容不再保存在浏览器本地
...
- 明确“本地 agent 在线”和“本地 agent 离线”两种模式下的页面行为
- 允许其他设备在本地 agent 离线时缓存待同步记录，但恢复连接后进入待同步队列，由用户手动确认再补交到本地
- 明确服务器端是否只做转发，不长期保存正文
- 明确失败场景：agent 不在线、网络失败、重复提交、覆盖提交
- 明确缓存内容的最小元数据，例如日期、正文、meta 状态、来源设备、最后编辑时间

完成标准：
- 网页可在服务器上使用
- 不依赖本地 `localStorage` 恢复正式数据
- 正文默认不直接落在公网服务器上
- 只有在能连接本地 agent 时，网页才能访问真实记录和执行正式写入
